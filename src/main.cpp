...
uint256 hashGenesisBlock("0xc23e8437fd645a81ff34dcb7cf3de5a045441e9a3212129f85275089888a373d");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20);
static CBigNum bnNoInstamine(~uint256(0) >> 25);
...
const string strMessageMagic = "PenguinCoin Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    {        
		...
		int rand = generateMTRandom(seed, 49999);

		nSubsidy = (rand + 1) * COIN;
    }
	else
    {
        std::string cseed_str = prevHash.ToString().substr(8,7);
    ...
    nSubsidy >>= (nHeight / 100000); //Half every 100k blocks
...
static const int64 nTargetTimespan = 150 * 15;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
	...
	if (pindexLast->nHeight+1 == 15)
		return bnNoInstamine.GetCompact();
    ...
    printf(" nActualTimespan = %"PRI64d" before bounds\n", nActualTimespan);        
         ...
         int64 nActualTimespanMax = ((nTargetTimespan*70)/50);
         int64 nActualTimespanMin = ((nTargetTimespan*50)/70);
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal PenguinCoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "PenguinCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "29th jan doing crypto";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 500000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("047ec8529d00a7e6bb72202339959ded470da2261f0a7bf976b3d3101aaa78c7af0cf8315f07ca1e74851f4cc696ec18df8c5ff8599edb4c106964335c7d9b0820") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1391040000;
        ...
        assert(block.hashMerkleRoot == uint256("0xd2c7df86b71b4e3c274899ba7e5b9ce160a3e4f0bb5d2fd554ce3730344a9b7d"));
...
unsigned char pchMessageStart[4] = { 0xfd, 0xca, 0xb4, 0xdc }; // PenguinCoin: increase each by adding 2 to bitcoin's value.
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // PenguinCoind: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
