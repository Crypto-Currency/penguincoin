...
SkinsPage::SkinsPage(QWidget *parent) : QWidget(parent), ui(new Ui::SkinsPage)
{
  ...
  QSettings settings("PenguinCoin", "settings");
...
void SkinsPage::loadSettings()
{
  QSettings settings("PenguinCoin", "settings");
//  inipath=settings.value("path", "").toString();
  std::string tmpini=GetDataDir().string() +"/themes";
  inipath=tmpini.c_str();
...
void SkinsPage::getlist()
{
  ...
  statusLabel->setText("<b>" + tr("Downloading themes from http://penguincoin.co...") + "</b>");
  latestNetError = "";
...
{
  if (netHandleError(reply, "http://penguincoin.co/themes/list.txt")) {
    ...
    for(int i=0;i<list.count();i++)
    {
      ...
      if(line.length())
      {  
        download("http://penguincoin.co/themes/"+line);
      } 
    }
  }
  else
